...
public:
    ...
    void AnalyzeImage()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            m_best_match_table_used[i].resize(4);
            for (int k = 0; k < 4; k++)
            {
    ...
    Individual CrossOver(Individual parent1, Individual parent2)
    {
        ...
        auto AddCandidatePiece = [&](const int &piece_id, const Position &position, const int &orientation) {
            if (piece_id < 0 || piece_id >= m_rows * m_columns) return;
            if (orientation < 0 || orientation >= 4) return;
            // Shared Edge
            {
        ...
        // Start with a random piece from parent1 at position (0,0)
        int root_piece = parent1.m_pieces[m_uid(m_rng)];
        AddPieceToKernel(root_piece, Position(0, 0));
        
        int max_iterations = m_rows * m_columns * 10; // Prevent infinite loop
        int iteration = 0;
        while (!candidate_piece.empty() && iteration++ < max_iterations)
        {
        ...
        pieces.resize(m_rows * m_columns, -1);  // Initialize with -1
        for (auto &kv : kernel)
        {
            Position position = kv.second;
        ...
        // Fill any unassigned positions with remaining pieces
        std::set<int> used_pieces(pieces.begin(), pieces.end());
        int next_piece = 0;
        for (int i = 0; i < pieces.size(); i++) {
            if (pieces[i] == -1) {
                while (used_pieces.count(next_piece)) {
                    next_piece++;
                }
                pieces[i] = next_piece;
                used_pieces.insert(next_piece);
                next_piece++;
            }
        }
    ...
    int m_rows, m_columns, m_piece_size, m_image_size;
