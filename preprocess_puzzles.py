...
def process_single_puzzle(
    ...
    for idx, piece in enumerate(pieces):
        ...
        # 1. Original
        cv2.imwrite(
            str(dirs['original'] / base_name),
            cv2.cvtColor(piece, cv2.COLOR_RGB2BGR)
        )
        
        # 2. Preprocessed (denoise + bilateral)
        piece_prep = preprocess(piece, method='full')
        cv2.imwrite(
            str(dirs['prep'] / base_name),
            cv2.cvtColor(piece_prep, cv2.COLOR_RGB2BGR)
        )
        ...
        binary = cv2.adaptiveThreshold(
            gray_prep, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
...
def main():
    ...
    parser.add_argument('-i', '--input-dir', required=True, help="Input puzzle directory")
    parser.add_argument('-o', '--output-dir', required=True, help="Output base directory")
    parser.add_argument('-g', '--grid', type=int, required=True, help="Grid size (e.g., 4 for 4x4)")
    parser.add_argument('-n', '--num-images', type=int, default=110, help="Number of images to process")
    ...
    success_count = 0
    for i in tqdm(range(args.start_id, args.start_id + args.num_images), desc="Processing"):
