...
def process_example(input_path: str, output_dir: str, puzzle_name: str):
    ...
    cv2.imwrite(
        os.path.join(output_dir, f"{puzzle_name}_1_original.jpg"),
        original,
    ...
    cv2.imwrite(
        os.path.join(output_dir, f"{puzzle_name}_2_upscaled.jpg"),
        upscaled_resized,
        [cv2.IMWRITE_JPEG_QUALITY, 90],
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, f"{puzzle_name}_3_binary.jpg"),
        binary_resized,
        [cv2.IMWRITE_JPEG_QUALITY, 90],
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, f"{puzzle_name}_4_edges.jpg"),
        edges_resized,
        [cv2.IMWRITE_JPEG_QUALITY, 90],
    )
    ...
    contours, _ = cv2.findContours(
        tile_binary.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    cv2.imwrite(
        os.path.join(output_dir, f"{puzzle_name}_5_contours.jpg"),
        contour_resized,
        [cv2.IMWRITE_JPEG_QUALITY, 90],
    )
